...
class BillForm extends ContentEntityForm {
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    switch ($status) {
      case SAVED_NEW:
        $this->messenger()->addMessage($this->t('Created the %label Bill.', [
          '%label' => $entity->label(),
        ]));
        break;

      default:
        $this->messenger()->addMessage($this->t('Saved the %label Bill.', [
          '%label' => $entity->label(),
        ]));
    }
    $form_state->setRedirect('entity.se_bill.canonical', ['se_bill' => $entity->id()]);
  }
