...
class CustomerController extends ControllerBase {
  ...
  public function revisionOverview(CustomerInterface $se_customer) {
    ...
    foreach (array_reverse($vids) as $vid) {
      /** @var \Drupal\se_customer\Entity\CustomerInterface $revision */
      ...
      if ($revision->hasTranslation($langcode) && $revision->getTranslation($langcode)->isRevisionTranslationAffected()) {
        ...
        if ($vid != $se_customer->getRevisionId()) {
          $link = Link::fromTextAndUrl($date, new Url('entity.se_customer.revision', [
            'se_customer' => $se_customer->id(),
            'se_customer_revision' => $vid,
          ]))->toString();
        }
        else {
          $link = $se_customer->toLink($date)->toString();
        }
