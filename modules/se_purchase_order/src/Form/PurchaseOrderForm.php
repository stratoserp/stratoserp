...
class PurchaseOrderForm extends ContentEntityForm {
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    switch ($status) {
      case SAVED_NEW:
        $this->messenger()->addMessage($this->t('Created the %label Purchase Order.', [
          '%label' => $entity->label(),
        ]));
        break;

      default:
        $this->messenger()->addMessage($this->t('Saved the %label Purchase Order.', [
          '%label' => $entity->label(),
        ]));
    }
    $form_state->setRedirect('entity.se_purchase_order.canonical', ['se_purchase_order' => $entity->id()]);
  }
